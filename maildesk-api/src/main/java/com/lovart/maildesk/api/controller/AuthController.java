 ...
 * {@code api-contract-v1.yaml} Profile schema; {@code /gmail/authorize} is a thin
...
public class AuthController {
    ...
    @GetMapping("/me")
    public ResponseEntity<ProfileDto> me(@AuthenticationPrincipal SessionPrincipal principal) {
        if (principal == null) {
            return ResponseEntity.status(401).build();
        }
        return ResponseEntity.ok(profiles.getCurrentProfile(principal.userId()));
    }
